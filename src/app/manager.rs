...
mod tests {
    ...
    fn it_can_add_and_remove_countries() {
        ...
        assert_eq!(add(MOCK_FILENAME, france.clone(), &countries), Output::Add(france.clone()));
        let expected_countries = Output::List(vec![france.clone()]);
        assert_eq!(list(MOCK_FILENAME, &countries), expected_countries);

        assert_eq!(add(MOCK_FILENAME, spain.clone(), &countries), Output::Add(spain.clone()));
        let expected_countries = Output::List(vec![france.clone(), spain.clone()]);
        assert_eq!(list(MOCK_FILENAME, &countries), expected_countries);

        assert_eq!(add(MOCK_FILENAME, greece.clone(), &countries), Output::Add(greece.clone()));
        let expected_countries = Output::List(vec![france.clone(), spain.clone(), greece.clone()]);
        assert_eq!(list(MOCK_FILENAME, &countries), expected_countries);

        assert_eq!(remove(MOCK_FILENAME, france.clone(), &countries), Output::Remove(france.clone()));

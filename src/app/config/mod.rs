...
lazy_static! {
    pub static ref JSON_PATH: String = {
        let directory_name = PKG_NAME.to_title_case().replace(" ", "");
        ...
        config_dir()
            .map(|dir| {
                ...
                match create_dir_all(&path) {
                    Ok(_) => format!("{path}{MAIN_SEPARATOR}{PKG_NAME}.json"),
                    Err(_) => format!("{PKG_NAME}.json"),
                }
            })
            .unwrap()
    };
